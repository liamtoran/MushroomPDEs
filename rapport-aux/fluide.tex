...
Autour de $(\mu,\rho,C,v) = (0,0,C_0,v)$, la premiere ligne donne: $-s\mu' + (\mu v)' = f_0(\mu + \rho)$ car on peut negliger $\mu\rho$ devant $\mu$ et $\rho$ . On a donc \begin{equation}
	(\mu v)'=(f_0\rho_0-\frac{sXf_0\rho_0}{F_0}-\frac{s^2X\rho_0}{F_0})\exp(Xy)
\end{equation}
donc \begin{equation}
	\mu v = (\frac{f_0\rho_0}{X}- \frac{C_0s\rho_0}{F_0}-\frac{s^2X\rho_0}{F_0})\exp(Xy).
\end{equation}
...
On peut ici négliger $\mu v\rho$ et $\mu C'$ devant $\mu v$. Ainsi, on a \begin{equation}
...
\end{equation} où $P_3(Y) \equiv   Y^3(\lambda + f_0)+ Y^2(f_0^2+ \lambda f_0 - f_0F_0)- Y(F_0f_0(\lambda+2f_0)) + f_0^2F_0^2$ est un polynôme de degré 3 dont les coefficients ne dépendent que des données $f_0$, $F_0$, $\lambda$ et $T$. \\
Soit $Y$ une racine de l'équation \ref{eq:PY}, on a alors \begin{equation}
	s^2 = -\frac{Y^4}{P_3(Y)}
\end{equation} 
...
On obtient alors un ou trois candidats négatifs pour $s$ par la formule: \begin{equation*}
	s^2 = -\frac{Y^4}{P_3(Y)}.
\end{equation*}
Le $s$ recherché est alors le plus grand de ces candidats: en effet, ce $s$ est le plus grand $s$ tel que les racines de \ref{eq:PY} soient toutes réelles.
 \ifdefined\COMPLETE
\else
\end{document}
\fi